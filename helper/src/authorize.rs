...
/// never harbour-teslacontrol's own PID - confirmed via `busctl list
...
/// org.teslacontrol.Helper. So the actual per-app gate is one layer up:
/// only an app whose .desktop file declares the TeslaControlHelper
...
pub fn authorize(
    ...
    if proc_uid != cred_uid {
        eprintln!(
            "teslacontrold: rejected call from pid {pid}: uid mismatch (proc={proc_uid} creds={cred_uid})"
        );
    ...
    let exe = proc.exe().map_err(|e| {
        ...
        // used to fail silently in the original Go implementation, which is
        // exactly what made the original "helper not found" bug invisible
        // in the journal.
        eprintln!(
            "teslacontrold: rejected call from pid {pid}: cannot resolve caller binary ({e})"
        );
    ...
    {
        eprintln!("teslacontrold: authorized call from pid {pid} ({exe})");
        return Ok(());
    }
    eprintln!(
        "teslacontrold: rejected call from pid {pid} ({exe}): not in TESLACONTROLD_ALLOWED_CALLERS"
    );
...
mod tests {
    ...
    fn test_split_and_trim() {
        let got = split_and_trim(" /usr/bin/harbour-teslacontrol, /opt/foo ,, , /x ");
        assert_eq!(got, vec!["/usr/bin/harbour-teslacontrol", "/opt/foo", "/x"]);
    }

    #[test]
    fn test_resolve_caller_paths() {
        let dir =
            std::env::temp_dir().join(format!("teslacontrold-authtest-{}", std::process::id()));
